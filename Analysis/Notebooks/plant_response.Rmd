...
baroverlap <- baroverlap.df %>% 
  ...
  geom_text(data = filter(baroverlap.df, Direction2 == "Up" & Set == "ArbuckleBiggsDavis"), aes(y = n - 30, label = n), hjust = 1, color = "white", angle = 90, size = 3.5) +
  geom_text(data = filter(baroverlap.df, Direction2 == "Down" & Set == "ArbuckleBiggsDavis"), aes(y = n - 30, label = n), hjust = 1, color = "black", angle = 90, size = 3.5) +
  scale_fill_manual(values = c("#8C510A", "#DFC27D", "#4D9221", "#B8E186"), guide = F) +
